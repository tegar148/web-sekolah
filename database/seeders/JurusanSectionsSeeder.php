...
class JurusanSectionsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($data as $page => $sections) {
            foreach ($sections as $sectionKey => $contentData) {
                $payload = [
                    'title' => $contentData['title'] ?? null,
                    'subtitle' => $contentData['subtitle'] ?? null,
                ];

                if (array_key_exists('button_text', $contentData)) {
                    $payload['button_text'] = $contentData['button_text'];
                }

                if (array_key_exists('content', $contentData)) {
                    $payload['content'] = json_encode($contentData['content']);
                }
