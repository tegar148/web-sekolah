...
<body class="bg-[#F8FAFC] antialiased text-gray-800">
    ...
    <section class="bg-gray-50 py-20 border-y border-gray-100">
        <div class="max-w-7xl mx-auto px-6 text-center mb-16">
            ...
            <p class="text-gray-500 max-w-2xl mx-auto">{{ $sections['prospek_karir']->subtitle ?? 'Lulusan memiliki kompetensi standar industri peternakan yang siap mengisi berbagai posisi strategis dan profesional.' }}</p>
        </div>
            ...
            @foreach($prospek_karir_data as $i => $role)
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 flex flex-col hover:-translate-y-1 transition duration-300">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-5
                        @if($i == 2) bg-green-50 text-green-600
                        @elseif($i == 3) bg-purple-50 text-purple-600
    ...
    <section class="max-w-6xl mx-auto px-6 py-20 overflow-hidden">
        <div class="mb-12 text-center md:text-left">
            <h2 class="text-3xl font-bold text-gray-900 mb-3">{{ $sections['fasilitas_jurusan']->title ?? 'Fasilitas Pembelajaran' }}</h2>
            <p class="text-gray-500 max-w-xl">{{ $sections['fasilitas_jurusan']->subtitle ?? 'Infrastruktur lengkap guna menyokong simulasi kerja industri yang berstandar nasional.' }}</p>
        </div>
    ...
    <section class="max-w-5xl mx-auto px-6 pb-20 mt-10">
        <div class="bg-[#2D3748] rounded-[2.5rem] p-12 md:p-16 text-center text-white relative overflow-hidden shadow-2xl">
            ...
            <div class="relative z-10">
                <h2 class="text-3xl md:text-4xl font-bold mb-6 tracking-tight">{{ $sections['cta']->title ?? 'Siap Menjadi Ahli Peternakan Modern?' }}</h2>
                <p class="text-gray-400 mb-10 max-w-lg mx-auto leading-relaxed text-sm">{{ $sections['cta']->subtitle ?? 'Pendaftaran peserta didik baru tahun ajaran 2024/2025 telah dibuka. Amankan karier masa depanmu di industri agroteknologi.' }}</p>
