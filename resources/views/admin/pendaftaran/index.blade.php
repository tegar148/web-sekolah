...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Data Pendataan Pra-PPDB</h1>
        <p class="text-sm text-gray-500 mt-1">Daftar masuk formulir Pra-PPDB. <span class="font-semibold text-slate-700">View only.</span></p>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100 text-[10px] uppercase tracking-widest text-gray-500">
                    ...
                    <th class="py-4 px-6 font-bold">Minat Jurusan</th>
                ...
                <tr class="hover:bg-slate-50/60 transition group">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-teal-500 to-cyan-700 text-white flex items-center justify-center font-bold text-sm shrink-0 shadow-sm">
                                {{ strtoupper(mb_substr($p->nama ?? 'A', 0, 1)) }}
                            </div>
                            <div>
                                <p class="font-bold text-gray-900 text-[13px]">{{ $p->nama }}</p>
                                <p class="text-[11px] text-gray-400 mt-0.5">
                                    {{ $p->jenis_kelamin ?? '—' }}
                                </p>
                            </div>
                    ...
                    <td class="py-4 px-6">
                        ...
                        <div class="flex items-center gap-1.5 mb-1">
                            <span class="text-[12px] text-gray-700 font-medium">{{ $p->minat_jurusan }}</span>
                        </div>
                        @else
...
function openDetail(id) {
    ...
    setTimeout(function () {
        ...
        html += section(1, 'Data Pribadi',
            ...
            + row('Minat Jurusan', d.minat_jurusan)
            + '</div>'
        );
